...
public class LqcDownLoadToCache {

	private Context mContext;
	private static LqcDownLoadToCache mInstance;
	
	private ArrayList<DownLoadTask> taskList;
	private HashMap<String, DownLoadTask> taskMap;
	
	private LqcDownLoadToCache(Context context) {
		mContext = context.getApplicationContext();
		taskList=new ArrayList<DownLoadTask>();
		taskMap=new HashMap<String, DownLoadTask>();
	}

	public static LqcDownLoadToCache get(Context context) {
		if (mInstance == null) {
			mInstance = new LqcDownLoadToCache(context);
		}
		return mInstance;
	}

	public void downloadStart(LqcDownLoadCallback listener,int taskID,final String weburl){
    ...
    class DownLoadTask{
    	...
    	private LqcDownLoadCallback callback;
    	
    	DownLoadTask(int taskID,LqcDownLoadCallback callback,String cachePath){
    		this.taskID=taskID;
    	...
    	public LqcDownLoadCallback getCallback(){
    		return callback;
    	}
	...
	public interface LqcDownLoadCallback{
	...
	private class MyDownLoad extends Thread {
		...
		public MyDownLoad(Context context,String weburl) {
			this.weburl = weburl;
			this.allPath = mContext.getCacheDir()+"/"+Utils.getMD5(weburl);
		}
